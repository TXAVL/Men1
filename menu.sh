...
check_update() {
    local silent=${1:-false}  # Tham số tùy chọn: true hoặc false
        ...
        case $? in
            1)
                if [ "$silent" = false ]; then
                    ...
                    echo -e "${YELLOW}Có phiên bản mới: $latest_version. (Không yêu cầu cập nhật do chạy nền)${NC}"
                fi
                ;;
            2)
                if [ "$silent" = false ]; then
...
ping_ip() {
    ...
    if [[ -z "$ip" ]]; then
        echo -e "${RED}Địa chỉ IP không được để trống.${NC}"
        return
    fi
    ping -c 4 "$ip"
}
...
auto_update_check() {
    while true; do
        ...
        check_update true  # Gọi check_update ở chế độ silent
    done
}
...
main() {
    show_warning
    check_and_install_packages
    check_update  # Kiểm tra cập nhật ngay khi chạy
